...
export class AIServiceClient {
  ...
  private generateContextualMockResponse(advisorName: string, userMessage: string, systemMessage: string): string {
    ...
    if (userMessage.toLowerCase().includes('financial') || userMessage.toLowerCase().includes('revenue')) {
    ...
    if (userMessage.toLowerCase().includes('risk') || userMessage.toLowerCase().includes('concern')) {
      return `${responses[1]} We should particularly focus on market risks and competitive threats in this evaluation.`;
    }
    
    if (userMessage.toLowerCase().includes('recommend') || userMessage.toLowerCase().includes('decision')) {
