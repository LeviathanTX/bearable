...
import { useConversationStore, useActiveConversation, useUIState } from '../../stores/conversationStore';
...
export const OptimizedAdvisoryConversation = memo(function OptimizedAdvisoryConversation({
  ...
  const { start: startPerf, end: endPerf } = usePerformanceMonitor('conversation-render', environment.isDebugMode());
  ...
  }, [conversationId, activeConversation, createConversation, setActiveConversation, initialMode, startPerf, endPerf]);
  ...
  const selectedAdvisorData = useMemo(() =>
    allAdvisors.filter(advisor =>
      activeConversation?.selectedAdvisors.includes(advisor.id)
    ),
  ...
  const handleSendMessage = useCallback(async (content: string) => {
    ...
    try {
      ...
      const advisorsToQuery = selectedAdvisorData.length > 0
        ? selectedAdvisorData
...
Please provide advice in your characteristic style and expertise.`;
  ...
  // Handle mode change
  const handleModeChange = useCallback((newMode: typeof conversationModes[0]) => {
